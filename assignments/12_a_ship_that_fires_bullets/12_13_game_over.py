...
def run_game():
    ...
    while True:
        ...
        if game_active:
            ...
            # Ship hit detection
            if pygame.sprite.spritecollideany(ship, aliens) or any(alien.rect.bottom >= screen.get_rect().bottom for alien in aliens):
                ships_left -= 1
                if ships_left > 0:
                    aliens = create_fleet(settings, screen)
                    bullets.empty()
        ...
        lives_surf = font.render(f"Lives: {ships_left}", True, (200, 200, 200))

...
def run_game():
    ...
    pygame.display.set_caption("Scoring Demo")
    ...
    while True:
        ...
        if not aliens:
            ...
            settings.alien_points = int(settings.alien_points * 1.2)
            bullets.empty()
            aliens = create_fleet(settings, screen)
            direction = 1
        ...
        level_surf = font.render(f"Level: {level}", True, (180, 200, 255))

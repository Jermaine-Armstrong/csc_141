...
def run_game():
    ...
    pygame.display.set_caption("High Score Tracking")
    ...
    while True:
        ...
        if not aliens:
            level += 1
            score += 50  # bonus for clearing wave
            if score > high_score:
                high_score = score
            bullets.empty()
            aliens = create_fleet(settings, screen)
        ...
        hud2 = font.render(f"High Score: {high_score}", True, (255, 215, 0))
        hud3 = font.render(f"Level: {level}", True, (180, 200, 255))
